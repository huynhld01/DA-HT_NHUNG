...
void Adafruit_GFX::print(char*data) {
...
int Adafruit_GFX::print(int number) {
...
void Adafruit_GFX::invertPixel(int16_t x, int16_t y) {
...
void Adafruit_GFX::writeLineInvert(int16_t x0, int16_t y0, int16_t x1, int16_t y1) {
  int16_t steep = abs(y1 - y0) > abs(x1 - x0);
  if (steep) {
    _swap_int16_t(x0, y0);
    _swap_int16_t(x1, y1);
...
void Adafruit_GFX::drawFastVLineInvert(int16_t x, int16_t y, int16_t h) {
  startWrite();
  writeLineInvert(x, y, x, y + h - 1);
  endWrite();
}
void Adafruit_GFX::writeFastVLineInvert(int16_t x, int16_t y, int16_t h) {
  // Overwrite in subclasses if startWrite is defined!
  // Can be just writeLine(x, y, x, y+h-1, color);
  // or writeFillRect(x, y, 1, h, color);
  drawFastVLineInvert(x, y, h);
}
void Adafruit_GFX::fillRectInvert(int16_t x, int16_t y, int16_t w, int16_t h) {
  startWrite();
  for (int16_t i = x; i < x + w; i++) {
...
void Adafruit_GFX::drawPixel(int16_t x, int16_t y, uint16_t color) {
    ...
    switch(rotation){
      ...
      if ((x < 0) || (x >= WIDTH) || (y < 0) || (y >= HEIGHT))
    return;
//    uint8_t *ptr = &pcd8544_buffer[(x / 8) + y * ((WIDTH + 7) / 8)];
